#!/usr/bin/env python3
"""Apply watermark fix to n8n_post_content.json"""
...
def main():
    ...
    for node in wf.get('nodes', []):
        if node.get('name') == 'Prepare Telegram Payload':
            new_code = '''// Prepare Telegram Payload
...
// FIX: If files is empty, construct path directly from storage structure
let watermarkedVideo = null;
if (files.length === 0) {
  const wmPath = `/storage/free-viral-shorts/${video_id}/watermarked`;
  watermarkedVideo = {
...
return [{
  json: { chat_id, video_id, video_path, video_download_url, caption, metadata_text, filename: watermarkedVideo.filename || video_path || video_download_url }
    ...
    print('Saved n8n_post_content.json')
